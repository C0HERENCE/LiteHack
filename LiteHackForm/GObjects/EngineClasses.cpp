...
{
	uint32 v2 = GameMemory.Read32(base+off_InternalIndex);
...
{
	uint64 v6 = GameMemory.Read64(base + off_Class);
...
{
	uint64 v18 = GameMemory.Read64(base + off_Outer);
...
{
	uint32_t v10 = GameMemory.Read32(base + off_ComparisonIndex);
...
{
	uint32_t v6 = GameMemory.Read32(base + off_Number);
